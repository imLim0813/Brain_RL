...
: 본 논문은 Twin delayed deep deterministic policy gradient 알고리즘을 사용하여 연속적인 운동 학습에 대해 연구하고, 학습된 모델과 학습되지 않은 모델을 비교함으 로써 강화학습 모델의 성능을 검증하였다.
<br>
: 이미지를 입력으로 하고 출력을 행동으로 내 놓는 end-to-end 모델을 모델링하기 위해 Deep Q Network의 Convolutional Neural Network 구조를 사용하였으며, 입력은 cropping 기법과 크기를 조절함으로써 전 처리를 하여 사용하였다. 정규 분포 함수에 타겟과 커서 간의 유클리디안 거리를 입 력으로 취해서 얻은 출력을 보상으로 사용하였다.
<br>
...
<br>
: 본 연구를 통해 학습된 모델로 VM을 진행하였을 때 뇌 데이터와 pearson correlation은 학습되지 않은 모델에 비해 전반적으로 유의미했다. 하지만 학습된 모델 은 뇌의 위계적인 정보처리를 보여주진 못하였다. 표현 유사 분석을 통해서 학습된 모델이 관심 영역 별 각 단의 위계적인 정보처리를 보여주진 못하였으나, 학습되지 않은 모델보다 뇌 데이터와 더욱 유사하다는 것을 알 수 있었다. 또한, 학습된 모델의 단이 깊어질수록 실험에 중요한 정보를 잘 표현하는 것으로 나타났다. 따라서, 실험과 동일한 환경을 학습시킨 심층 강화학습 모델이 학습되지 않은 심층 강화학습 모델보다 연속적인 운동학습 실험을 통해 얻은 뇌 데이터를 더욱 잘 설명하는 것으로 해석된다.
